...
export type ManagerMessages = {
  ...
  cards: {
    ...
    section: string; title: string; description: string; responseTime: (ms: number) => string;
    evidenceAria: string; verified: string; claimed: string; flagsAria: string;
  ...
  comparison: {
    ...
    firstOnly: (names: string) => string; unchanged: string; count: (count: number) => string; onlyHere: string;
    availabilityRemoved: (count: number) => string; cancelled: string; failed: string;
  };
  json: { title: string };
...
const ru: ManagerMessages = {
  ...
  presets: { title: "Демо-сценарии", hint: "Заполняют и сразу запускают", items: [{ label: "Ведущий", hint: "Плотная категория" }, { label: "Флорист", hint: "Меньше трёх" }, { label: "Лайв-бэнд", hint: "Нет категории" }, { label: "Декоратор", hint: "Все заняты" }] },
  pipeline: {
    section: "02", title: "Поток решений", live: "События подбора", ai: "ИИ", code: "Код", emptyTitle: "Пайплайн ждёт запрос", emptyDescription: "Выберите один из четырёх сценариев — здесь появятся реальные этапы обработки без скрытого спиннера.",
  ...
  cards: {
    ...
    section: "04", title: "Карточки с доказательствами", description: "Каждое утверждение помечено источником.", responseTime: (ms) => `Ответ получен за ${(ms / 1000).toFixed(2)} с`, evidenceAria: "Факты в объяснении", verified: "Проверено", claimed: "Со слов подрядчика", flagsAria: "Особенности данных", synthetic: "Синтетический профиль", cityImputed: "Город проставлен", priceImputed: "Цена проставлена",
  ...
  comparison: { section: "05", title: "Один запрос — две даты", description: "Сравним дату из формы с соседней датой через неделю, сохранив остальные условия. Воронка покажет точный отсев по занятости, карточки — причины выбора.", action: "Сравнить с другой датой", loading: "Сравниваем…", changed: (count) => `Различаются ${count} карточки`, firstOnly: (names) => `Только в первой выдаче: ${names}. Причина смены состава видна в отсевах по датам ниже.`, unchanged: "Все карточки первой выдачи остались и во второй.", count: (count) => `${count} в выдаче`, onlyHere: "Только здесь", availabilityRemoved: (count) => `На этапе занятости исключено: ${count}`, cancelled: "Сравнение отменено.", failed: "Не удалось сравнить две даты." },
...
const kk: ManagerMessages = {
  ...
  cards: { ...ru.cards, photo: "Фото: ЖИ иллюстрациясы", photoMissing: "Фото қолжетімсіз", title: "Дәлелдері бар карточкалар", description: "Әр тұжырымның дереккөзі белгіленген.", responseTime: (ms) => `Жауап ${(ms / 1000).toFixed(2)} с ішінде алынды`, evidenceAria: "Түсіндірмедегі фактілер", verified: "Тексерілген", claimed: "Мердігердің айтуынша", flagsAria: "Дерек ерекшеліктері", synthetic: "Синтетикалық профиль", cityImputed: "Қала толықтырылды", priceImputed: "Баға толықтырылды", outcomes: { found: { title: "Сәйкес мердігерлер табылды", label: "Нәтиже бар" }, no_category_in_city: { title: "Бұл қалада мұндай санат жоқ", label: "Санат жоқ" }, all_filtered_out: { title: "Барлық үміткер шарттардан өтпеді", label: "Барлығы шығарылды" } } },
  comparison: { section: "05", title: "Бір сұрау — екі күн", description: "Формадағы күнді арасы бір апта басқа күнмен салыстырамыз. Қалған шарттар өзгермейді; сүзгі бос емес үміткерлердің санын көрсетеді.", action: "Басқа күнмен салыстыру", loading: "Салыстырып жатырмыз…", changed: (count) => `${count} карточка ерекшеленеді`, firstOnly: (names) => `Тек бірінші нәтижеде: ${names}. Құрамның өзгерісін төмендегі күн сүзгісінен қараңыз.`, unchanged: "Бірінші нәтижедегі барлық карточка екінші нәтижеде де бар.", count: (count) => `Нәтижеде ${count}`, onlyHere: "Тек мұнда", availabilityRemoved: (count) => `Бос еместік сүзгісі алып тастағаны: ${count}`, cancelled: "Салыстыру тоқтатылды.", failed: "Екі күнді салыстыру мүмкін болмады." },
...
const en: ManagerMessages = {
  ...
  presets: { title: "Demo scenarios", hint: "Fill and run instantly", items: [{ label: "Host", hint: "Dense category" }, { label: "Florist", hint: "Fewer than three" }, { label: "Live band", hint: "No category" }, { label: "Decorator", hint: "All unavailable" }] },
  pipeline: { ...ru.pipeline, ai: "AI", code: "Code", live: "Matching events", title: "Decision stream", emptyTitle: "The pipeline is waiting", emptyDescription: "Choose one of four scenarios to see real processing stages instead of a hidden spinner.", waitingTitle: "Waiting for the next step", waitingDescription: "The connection is open; data arrives when ready.", interrupted: "Stream interrupted", retry: "Retry", aria: "Event stream", step: "Step", criteriaTitle: "Criteria generated", criteriaDetail: (count) => `${count} criteria for this request`, filterDetail: (before, after, reason) => `${before} → ${after}. ${reason}`, rankedTitle: "Ranking calculated", rankedOrder: (ids) => `Order: ${ids.join(", ")}`, rankedEmpty: "No candidates remained after filtering", cardTitle: (name) => `Card: ${name}`, cardDetail: (count) => `${count} facts in the explanation`, criticOk: "Critic accepted the explanations", criticProblems: "Critic found issues", criticOkDetail: "Every claim is grounded in facts", criticProblemsDetail: (count) => `${count} issues`, doneTitle: "Response ready", doneDetail: (outcome, count) => `${outcome}: ${count} cards` },
  ...
  cards: { ...ru.cards, photo: "Photo: AI illustration", photoMissing: "Photo unavailable", title: "Evidence-backed cards", description: "Every statement is marked with its source.", responseTime: (ms) => `Response received in ${(ms / 1000).toFixed(2)} s`, evidenceAria: "Facts used in the explanation", verified: "Verified", claimed: "Contractor claim", flagsAria: "Data flags", synthetic: "Synthetic profile", cityImputed: "City imputed", priceImputed: "Price imputed", outcomes: { found: { title: "Matching contractors found", label: "Results found" }, no_category_in_city: { title: "This category is unavailable in the city", label: "No category" }, all_filtered_out: { title: "All candidates were filtered out", label: "All filtered out" } } },
  comparison: { section: "05", title: "One request, two dates", description: "Compare the date in the form with one a week apart, keeping every other condition. The funnel shows availability exclusions; the cards show why each choice was made.", action: "Compare another date", loading: "Comparing…", changed: (count) => `${count} cards differ`, firstOnly: (names) => `Only in the first result: ${names}. See the date-filter counts below for the availability impact.`, unchanged: "Every card from the first result remains in the second.", count: (count) => `${count} in results`, onlyHere: "Only here", availabilityRemoved: (count) => `Excluded at the availability step: ${count}`, cancelled: "Comparison cancelled.", failed: "Could not compare the two dates." },
